...
namespace{
	...
	void print_vec(string str, double* vec, int n)
	{
		cout << endl << str << " ";
	...
	void print_mat(string str, double mat[3][3])
	{
		cout << endl << str << " ";
	...
	int ap3p::computePoses(const double featureVectors[3][3],
		...
		double aaa[5] = { f21,f22,f23,f24,f25 };
		...
		for (int i = 0; i < 4; ++i) {
			...
			print_vec("opencv T", solutionsT[nb_solutions], 3);
	...
	int ap3p::solve(double R[4][3][3], double t[4][3], double mu0, double mv0, double X0, double Y0, double Z0, double mu1,
		...
		cout << "opencl B3 = " << mu1 << " " << mv1 << " " << mk1 << endl;
